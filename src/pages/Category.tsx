...
const Category = () => {
  ...
  const COLOR_OPTIONS = [
    { name: "Kahverengi", class: "#4F4631" },
    { name: "Yeşil", class: "#314F4A" },
    { name: "Mavi", class: "#31344F" },
  ];
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 text-left font-satoshi">
        ...
        <div className="flex gap-16 items-start">
          <aside className="hidden md:flex flex-col w-[300px] shrink-0 sticky top-32 max-h-[calc(100vh-160px)] overflow-y-auto scrollbar-hide border-2 border-zinc-100 rounded-[40px] p-8 space-y-10 bg-[#FAFAFA] shadow-sm">
                ...
                {COLOR_OPTIONS.map((color) => (
                  <button
                    ...
                    onClick={() =>
                      setTempFilters((prev) => ({
                        ...prev,
                        color: prev.color === color.class ? null : color.class,
                      }))
                    }
                    style={{ backgroundColor: color.class }}
                    className={`w-8 h-8 rounded-full border-2 transition-all duration-300 hover:scale-110 shadow-sm ${tempFilters.color === color.class ? "border-black ring-2 ring-zinc-200 scale-110" : "border-transparent opacity-80"}`}
                    title={color.name}
                  />
                ))}
              </div>
            ...
            <div className="sticky bottom-0 bg-[#FAFAFA] pt-4 pb-2 z-10">
              <Button
